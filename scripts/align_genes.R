...
# out_fasta <- '../mtGenome/aligned_fish_genes.fasta'
...
aligned_gene_sequences <- gene_sequences %>%
  # filter(gene %in% c('ATP8', 'ND4L')) %>%
  rowwise %>%
  mutate(alignment = list(msa(sequence, type = 'DNA', verbose = TRUE, order = 'input') %>% DNAStringSet()))

#### Concatenate Coding Sequence ####
concatenated_coding_sequence <- aligned_gene_sequences %>%
  pull(alignment) %>%
  map(as.character) %>%
  do.call(paste0, .) %>%
  set_names(names(aligned_gene_sequences$alignment[[1]])) %>%
  DNAStringSet()

#### Write FASTA for tree making ####
writeXStringSet(concatenated_coding_sequence, out_fasta)
